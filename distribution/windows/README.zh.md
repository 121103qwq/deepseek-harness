...
安装程序会在“这是社区分发版本，不是 DeepSeek 官方安装程序”提示之后提供“选择插件…”按钮；勾选结果只写入当前用户的 Web profile。`deepseek-desktop-plugin-helper` 是两个安装模式都必须自带的插件，提供只读的本地插件清单接口 `/__deepseek_desktop/plugin-helper`。在线版会在安装阶段下载已固定来源的网络插件；离线版会保留这些条目但默认关闭。本次只提交源码，不重新打包 setup：
