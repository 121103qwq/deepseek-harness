...
- **不能任意输入推理强度**——composer 滑杆仅按适配器顺序提供确切模型公布的推理强度；适配器没有推理元数据时不显示 Effort 行。
