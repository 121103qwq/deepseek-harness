...
模型、Agent、工作流、MCP、工具、权限、备份和更新导航建立了管理器的资源词汇；后续可以增加针对运行时的编辑器，而不改变注册表和隔离规则。管理器单独构建和分发；DeepSeek Desktop 安装程序不会把它嵌入载荷。两个应用采用相同的规范化 `DSH_HOME` 启动锁约定，防止同时修改同一个 Harness Home。桌面打包决策记录在 [Windows 离线 DeepSeek Desktop 安装程序](2026-08-26-windows-offline-desktop-installer.md)。
