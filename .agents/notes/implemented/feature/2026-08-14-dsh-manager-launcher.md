...
The Models, Agent, workflow, MCP, tools, permissions, backup, and update navigation establishes the manager's resource vocabulary; runtime-specific editors can be added without changing the registry or isolation rule. The installer includes the manager beside DeepSeek Desktop and adds a Start menu shortcut, while no GitHub repository or release is created by this change.
